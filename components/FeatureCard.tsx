...
export default function FeatureCard({
  ...
  if (variant === "teal") {
    return (
      <div className={`md:col-span-2 relative overflow-hidden bg-brand-teal text-white rounded-3xl p-8 sm:p-10 shadow-lg shadow-brand-teal/10 hover:shadow-brand-teal/20 transition-all duration-300 hover:scale-[1.01] group flex flex-col justify-between min-h-[300px] ${className}`}>
        ...
        <div className="w-12 h-12 bg-teal-800/40 rounded-full flex items-center justify-center mb-6">
          {icon}
        </div>
        <div>
          <h3 className="text-2xl font-bold mb-3">{title}</h3>
          <p className="text-teal-100/90 leading-relaxed text-sm max-w-xl font-medium">
            {description}
          </p>
        </div>
      </div>
  ...
  if (variant === "orange") {
    return (
      <div className={`bg-[#f59e0b] text-[#7c2d12] rounded-3xl p-8 shadow-lg shadow-amber-500/10 hover:shadow-amber-500/20 transition-all duration-300 hover:scale-[1.01] flex flex-col justify-between min-h-[300px] ${className}`}>
        <div className="w-12 h-12 bg-amber-600/20 rounded-full flex items-center justify-center mb-6 text-[#7c2d12]">
          {icon}
        </div>
        <div>
          <h3 className="text-xl font-bold mb-3">{title}</h3>
          <p className="text-[#7c2d12]/90 leading-relaxed text-sm font-semibold">
            {description}
          </p>
        </div>
      </div>
