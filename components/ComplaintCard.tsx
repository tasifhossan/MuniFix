...
import { MapPin, Clock, ThumbsUp, ThumbsDown, MessageSquare } from "lucide-react";
...
interface ComplaintCardProps {
  ...
  time: string;
  image?: string;
  avatars?: string[]; // kept for backward compatibility if any
  upvotes?: number;
  downvotes?: number;
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-4 pt-2 border-t border-slate-50">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold text-gray-500">
              ...
              {upvotes !== undefined && (
                <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Upvotes">
                  <ThumbsUp className="w-3.5 h-3.5 text-emerald-500" />
                  <span className="text-gray-700 font-bold">{upvotes}</span>
                </span>
              )}

              {/* Downvotes Counter Badge */}
              {downvotes !== undefined && (
                <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Downvotes">
                  <ThumbsDown className="w-3.5 h-3.5 text-rose-500" />
                  <span className="text-gray-700 font-bold">{downvotes}</span>
                </span>
              )}

              {/* Comments Counter Badge */}
              {commentsCount !== undefined && (
                <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Comments">
                  <MessageSquare className="w-3.5 h-3.5 text-brand-teal" />
                  <span className="text-gray-700 font-bold">{commentsCount}</span>
                </span>
              )}
            </div>
            <div>
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-50">
        <div className="flex items-center gap-2 text-xs font-semibold text-gray-500">
          ...
          {upvotes !== undefined && (
            <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Upvotes">
              <ThumbsUp className="w-3.5 h-3.5 text-emerald-500" />
              <span className="text-gray-750 font-bold">{upvotes}</span>
            </span>
          )}
          ...
          {downvotes !== undefined && (
            <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Downvotes">
              <ThumbsDown className="w-3.5 h-3.5 text-rose-500" />
              <span className="text-gray-750 font-bold">{downvotes}</span>
            </span>
          )}
          ...
          {commentsCount !== undefined && (
            <span className="flex items-center bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 gap-1 text-[11px]" title="Comments">
              <MessageSquare className="w-3.5 h-3.5 text-brand-teal" />
              <span className="text-gray-750 font-bold">{commentsCount}</span>
            </span>
          )}
        </div>
